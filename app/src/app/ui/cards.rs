...
impl SqueezeItApp {
    ...
    pub(super) fn footer(&mut self, area: Rect, buf: &mut Buffer) {
        let running = self.running.load(Ordering::SeqCst);
        let has_backups = self.has_backups();
        let mut spans = Vec::new();
        let mut x = area.x;
        let mut hits = std::mem::take(&mut self.hits);

        let mut chip = |k: &str, label: &str, code: Option<KeyCode>| {
        ...
        if running {
            chip("c", "cancel", Some(KeyCode::Char('c')));
        } else {
            chip("o", "select folder", Some(KeyCode::Char('o')));
            chip("f", "select files", Some(KeyCode::Char('f')));
            chip("s", "start", Some(KeyCode::Char('s')));
            if has_backups {
                chip("r", "restore", Some(KeyCode::Char('r')));
            }
        }
        chip("a", "settings", Some(KeyCode::Char('a')));
        if self.advanced_open {
            chip("↑↓", "move", None);
            chip("←→", "change", None);
        }
        chip("q", "quit", Some(KeyCode::Char('q')));
