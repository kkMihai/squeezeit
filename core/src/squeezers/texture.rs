...
pub fn power_of_two_floor(n: u32) -> u32 {
    if n <= MIN_DIMENSION {
...
fn block_floor(n: u32) -> u32 {
    (n & !3).max(MIN_DIMENSION)
}
...
pub fn target_dimensions(
    ...
    if !policy.allow_resize {
        ...
        return (block_floor(width), block_floor(height));
    }

    let w = power_of_two_floor(width);
    let h = power_of_two_floor(height);
    ...
    if matches!(role, TextureRole::Livery | TextureRole::Weapon) && policy.liveries.protects() {
        cap = cap.max(w.max(h));
    }
...
mod tests {
    ...
    use crate::settings::{SizeLimit, SqueezeSettings};
    use image::RgbaImage;
    ...
    #[test]
    fn snaps_down_to_a_power_of_two() {
        assert_eq!(power_of_two_floor(1000), 512);
        assert_eq!(power_of_two_floor(1500), 1024);
        assert_eq!(power_of_two_floor(1600), 1024);
        assert_eq!(power_of_two_floor(640), 512);
        assert_eq!(power_of_two_floor(2048), 2048);
        assert_eq!(power_of_two_floor(1), MIN_DIMENSION);
    }

    #[test]
    fn a_side_is_never_rounded_up() {
        for n in (MIN_DIMENSION..=4096).chain([5000, 6000, 8191]) {
            assert!(power_of_two_floor(n) <= n, "{n} was rounded up");
        }
    }

    #[test]
    fn no_role_or_preset_can_grow_a_texture() {
        let sizes = [(1600, 1600), (999, 512), (2048, 1080), (100, 100)];
        let roles = [
            TextureRole::Diffuse,
            TextureRole::Normal,
        ...
        for limit in [SizeLimit::Keep, SizeLimit::Max(512), SizeLimit::Max(4096)] {
            ...
            for family in [
                ...
                let p = policy(&s, family);
                for (w, h) in sizes {
                    for role in roles {
                        let (tw, th) = target_dimensions(w, h, role, &p);
                        assert!(
                            tw <= w.max(MIN_DIMENSION) && th <= h.max(MIN_DIMENSION),
                            "{family:?}/{role:?} grew {w}x{h} to {tw}x{th} under {limit:?}"
                        );
                    }
                }
            }
    ...
    #[test]
    fn a_keep_limit_only_snaps_to_a_block_boundary() {
        let s = SqueezeSettings {
        ...
        assert_eq!(
            target_dimensions(1000, 1000, TextureRole::Diffuse, &p),
            (1000, 1000)
        );
        assert_eq!(
            target_dimensions(999, 999, TextureRole::Diffuse, &p),
            (996, 996)
        );
        assert_eq!(
            target_dimensions(1, 1, TextureRole::Diffuse, &p),
    ...
    #[test]
    fn a_keep_limit_keeps_source_dimensions() {
        let s = SqueezeSettings {
            preset: crate::settings::Preset::Hair,
            size_limit: SizeLimit::Keep,
            ..Default::default()
        };
        assert_eq!(
    ...
    #[test]
    fn liveries_and_weapons_resize_once_the_protection_is_lifted() {
        let s = SqueezeSettings {
            size_limit: SizeLimit::Max(512),
            liveries: crate::settings::Liveries::Include,
            ..Default::default()
        };
        let p = policy(&s, AssetFamily::Generic);
        assert_eq!(
