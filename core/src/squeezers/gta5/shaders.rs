...
fn family_of_shader(name_hash: u32) -> Option<AssetFamily> {
    if PED_HAIR_SHADERS.contains(&name_hash) {
        Some(AssetFamily::PedHair)
    } else if VEHICLE_SHADERS.contains(&name_hash) {
...
fn shader_list(sys: &[u8], draw_off: usize) -> Option<(usize, usize)> {
    let group = system_offset(u64_at(sys, draw_off + 0x10)?, sys.len())?;
    let list = system_offset(u64_at(sys, group + 0x10)?, sys.len())?;
    Some((list, (u16_at(sys, group + 0x18)? as usize).min(1024)))
}

fn shader_name_hashes(sys: &[u8], draw_off: usize) -> impl Iterator<Item = u32> + '_ {
    let (list, count) = shader_list(sys, draw_off).unwrap_or((0, 0));
    (0..count).filter_map(move |i| {
        let shader = system_offset(u64_at(sys, list + i * 8)?, sys.len())?;
    ...
    for &draw in draw_offsets {
        for hash in shader_name_hashes(sys, draw) {
            match family_of_shader(hash) {
                ...
                Some(AssetFamily::PedHair) => ped_hair = true,
                Some(AssetFamily::PedCloth) => ped_cloth = true,
                _ => {}
            }
        }
    }
    match (ped_hair, ped_cloth) {
        (true, _) => AssetFamily::PedHair,
        (false, true) => AssetFamily::PedCloth,
        _ => AssetFamily::Generic,
    }
}
...
mod tests {
    ...
    #[test]
    fn hair_shaders_are_their_own_family() {
        for shader in [
            b"ped_hair_cutout_alpha".as_slice(),
            b"ped_hair_spiked",
            b"ped_fur",
        ] {
            assert_eq!(
                family_of_shader(joaat(shader)),
                Some(AssetFamily::PedHair),
                "{}",
                String::from_utf8_lossy(shader)
            );
            assert!(
                !PED_CLOTH_SHADERS.contains(&joaat(shader)),
                "hair shader must not also be cloth"
            );
        }
    }
}
