...
use crate::policy::{FormatRule, MipRule, Policy};
...
pub(super) fn optimize_texture(
    ...
    let mip_rule = if policy.mip_exception {
        ...
        if policy.mip_exception_applies(opaque, out_dds == dds, rec.levels, tw, th, in_place) {
    ...
    let gen_mips = mip_rule == MipRule::GenerateFull && !in_place;
...
fn choose_format(
    ...
    let promoted = match policy.format {
        ...
        FormatRule::Conservative => match dds {
            ImageFormat::BC2RgbaUnorm | ImageFormat::BC3RgbaUnorm if !in_place => {
