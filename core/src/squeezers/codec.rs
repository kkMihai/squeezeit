...
pub(crate) fn resize(image: RgbaImage, width: u32, height: u32) -> RgbaImage {
    if (width, height) == (image.width(), image.height()) {
        return image;
    }
    resize_simd(&image, width, height)
        .unwrap_or_else(|| imageops::resize(&image, width, height, imageops::FilterType::Lanczos3))
}

fn resize_simd(image: &RgbaImage, width: u32, height: u32) -> Option<RgbaImage> {
...
pub(crate) fn decode_block(
    ...
    surface.decode_rgba8().ok()?.to_image(0).ok()
}
...
pub(crate) fn resize_and_encode(
    ...
    let image = match gpu {
        Some(ctx) => {
            ...
            match gpu::resize_and_compress(
                ...
                Ok(data) => {
                    tracing::trace!(width, height, mip_levels, "texture compressed on GPU");
                    return Some(Surface {
                        width,
                        height,
                        depth: 1,
    ...
    let image = resize(image, width, height);
    encode_block(&image, format, quality, mip_levels)
}
...
pub(crate) fn resize_and_encode_from_blocks(
    ...
    match gpu::resize_and_compress_bc(
        ...
        Err((error, _)) => {
            ...
            let image = resize(image, width, height);
            encode_block(&image, out_format, quality, mip_levels)
        }
    }
}
