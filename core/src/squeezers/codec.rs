...
fn is_opaque_alpha_block(bytes: &[u8], format: ImageFormat) -> bool {
    match format {
        ImageFormat::BC2RgbaUnorm => bytes[..8].iter().all(|&b| b == 0xFF),
        ImageFormat::BC3RgbaUnorm => {
            bytes[0] == 0xFF && bytes[1] == 0xFF && bytes[2..8].iter().all(|&b| b == 0)
        }
        _ => false,
    }
}
...
pub(crate) fn bc_alpha_summary(
    ...
    for by in 0..bh {
        for bx in 0..bw {
            ...
            if is_opaque_alpha_block(block, format) {
                continue;
            }
...
mod tests {
    ...
    #[test]
    fn the_opaque_shortcut_agrees_with_expanding_the_block() {
        for format in [ImageFormat::BC2RgbaUnorm, ImageFormat::BC3RgbaUnorm] {
            for alpha in [
                (|_, _| 255) as fn(u32, u32) -> u8,
                |x, _| if x % 2 == 0 { 0 } else { 255 },
                |x, _| (x * 16).min(255) as u8,
                |_, y| if y < 2 { 200 } else { 255 },
            ] {
                let image = alpha_image(4, 4, alpha);
                let blocks = encode_block(&image, format, Quality::Slow, 1)
                    .expect("encode")
                    .data;
                let shortcut = is_opaque_alpha_block(&blocks, format);
                let expanded = match format {
                    ImageFormat::BC2RgbaUnorm => bc2_alpha_block(&blocks[..8]),
                    _ => bc3_alpha_block(&blocks[..8]),
                };
                let really_opaque = expanded.iter().all(|&a| a == 255);
                assert!(
                    !shortcut || really_opaque,
                    "{format:?}: the shortcut called a block opaque that is not {expanded:?}"
                );
            }
        }
    }
