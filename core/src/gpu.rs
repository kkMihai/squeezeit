...
use crate::settings::{ProcessingBackend, Quality};
...
const MAX_BATCH_JOBS: usize = 16;
const MAX_BATCH_OUTPUT_BYTES: u64 = 96 << 20;
const MAX_BATCH_INPUT_BYTES: u64 = 256 << 20;

const MAX_BATCH_COST: u64 = 24 << 20;
...
impl GpuContext {
    ...
    pub fn create_standalone() -> Result<Self, GpuError> {
        ...
        let adapter = request(wgpu::Backends::VULKAN)
            .or_else(|_| request(wgpu::Backends::from_env().unwrap_or_default()))?;
...
pub fn select(backend: ProcessingBackend, gpu: Option<&GpuContext>) -> Option<&GpuContext> {
    match backend {
    ...
    for job in jobs {
        ...
        if let Some(chain) = &chain_texture {
            for level in 0..job.mip_levels {
                ...
                let target_view = mip_view(chain, level);
