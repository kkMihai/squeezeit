...
use squeezeit::{Backend, GpuContext, SqueezeReport, SqueezeSettings, SqueezerRegistry, batch};
...
fn settings_for(backend: Backend) -> SqueezeSettings {
    SqueezeSettings {
        dry_run: true,
        make_backup: false,
        backend,
...
fn run_once(targets: &[PathBuf], root: &Path, registry: &SqueezerRegistry, backend: Backend) {
...
fn bench_single_texture(c: &mut Criterion) {
    ...
    for &side in &[1024u32, 2048] {
        ...
        let cpu_registry = SqueezerRegistry::new(None, None);
        group.bench_with_input(BenchmarkId::new("cpu", side), &side, |b, _| {
            b.iter(|| run_once(&targets, &target, &cpu_registry, Backend::Cpu));
        });

        if let Some(ctx) = gpu() {
            let gpu_registry = SqueezerRegistry::new(Some(ctx), None);
            run_once(&targets, &target, &gpu_registry, Backend::Gpu);
            group.bench_with_input(BenchmarkId::new("gpu", side), &side, |b, _| {
                b.iter(|| run_once(&targets, &target, &gpu_registry, Backend::Gpu));
            });
        }
...
fn bench_batch_throughput(c: &mut Criterion) {
    ...
    group.bench_function(
        ...
        |b| {
            b.iter(|| run_once(&targets, &dir, &cpu_registry, Backend::Cpu));
        },
    );

    if let Some(ctx) = gpu() {
        let gpu_registry = SqueezerRegistry::new(Some(ctx), None);
        run_once(&targets, &dir, &gpu_registry, Backend::Gpu);
        group.bench_function(
            BenchmarkId::new("gpu", format!("{total_files}x{side}")),
            |b| {
                b.iter(|| run_once(&targets, &dir, &gpu_registry, Backend::Gpu));
            },
        );
    }
...
fn bench_collect_targets(c: &mut Criterion) {
    ...
    let registry = SqueezerRegistry::new(None, None);
...
fn bench_resize(c: &mut Criterion) {
    ...
    for &(from, to) in &[(2048u32, 1024u32), (1024, 512), (4096, 1024)] {
        let src = RgbaImage::from_fn(from, from, |x, y| {
            image::Rgba([
                (x & 0xff) as u8,
                (y & 0xff) as u8,
                ((x ^ y) & 0xff) as u8,
        ...
        group.bench_with_input(
            BenchmarkId::from_parameter(format!("{from}to{to}")),
            &to,
            |b, &to| b.iter(|| squeezeit::codec::resize(src.clone(), to, to)),
        );
    }
    group.finish();
}
